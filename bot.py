...
async def cmd_arena(m: types.Message, command: CommandObject = None):
    ...
    # Initialize Pari-Mutuel Pool
    ACTIVE_ARENAS[m.chat.id] = {
        "status": "booking",
        "red_name": red_name,
        "blue_name": blue_name,
        "red_bets": {m.from_user.id: {"bet": bet_amount, "name": m.from_user.first_name}},
        "blue_bets": {}
    }
    ...
    sent_msg = await m.answer(
        ...
        f"💰 <b>Total Pool:</b> {bet_amount:,} Gold\n"
...
async def handle_arena_bet(call: types.CallbackQuery):
    ...
    # Ask for amount (For simplicity, we match the initiator's bet amount. You can modify this to a fixed fee later if desired)
    # To keep the fast pace, anyone clicking the button automatically matches the lowest bet, or let's use a flat 10k for joiners.
    # Let's make them bet a flat amount, or just grab their vault balance. Let's do a fixed 10% of their vault or flat 5k.
    # ACTUALLY, simpler: They just match the initiator's bet size.
    bet_amount = list(arena['red_bets'].values())[0]['bet'] 
