...
heist_games = {}
racing_games = {}
ACTIVE_TOWER_GAMES = {} # 👈 ADD THIS: Tracks active /tower climbs

TOWER_MULTIPLIERS = [0, 1.5, 2.5, 4.0, 7.5, 15.0, 30.0] # The exact 6-floor ladder

# --- ARENA DICTIONARIES & RNG ---
ACTIVE_ARENAS = {} # Tracks active fights per chat

ARENA_ADJECTIVES = ["Venomous", "Silent", "Crimson", "Iron", "Phantom", "Mad", "Savage", "Brutal", "Ruthless"]
ARENA_NOUNS = ["Butcher", "Ghost", "Viper", "Titan", "Reaper", "Hound", "Demon", "Slayer", "Beast"]

def get_mma_move(fighter_name):
    """Returns a tuple: (Commentary String, Points Scored)"""
    roll = random.random()
    if roll < 0.50: 
        move = random.choice(["lands a crisp jab", "connects with a low leg kick", "escapes the clinch"])
        return f"{fighter_name} {move}! (+1)", 1
    if roll < 0.80: 
        move = random.choice(["scores a heavy double-leg takedown", "lands a brutal overhand right", "connects with a spinning back elbow"])
        return f"{fighter_name} {move}! (+2)", 2
    if roll < 0.95: 
        move = random.choice(["lifts and slams them into the mat", "lands a devastating head kick", "secures a tight Kimura lock"])
        return f"{fighter_name} {move}! (+4)", 4
    
    move = random.choice(["connects with a flying knee", "sinks in a deep Rear Naked Choke"])
    return f"💥 <b>CRITICAL:</b> {fighter_name} {move}! (+5)", 5
...
async def update_player_bank(user_id: int, amount: int):
    """Updates the player's bank balance in RAM and queues it for the DB worker."""
    p = await get_player_fast(types.User(id=user_id, is_bot=False, first_name="User"))
    
    # Initialize bank_balance if it doesn't exist
    current_bank = p.get('bank_balance', 0)
    new_bank = current_bank + amount
    
    # Update RAM
    p['bank_balance'] = new_bank
    
    # Queue for Database Worker
    if 'players' not in DB_QUEUE:
        DB_QUEUE['players'] = {}
    
    if user_id not in DB_QUEUE['players']:
        DB_QUEUE['players'][user_id] = {"_match_col": "user_id"}
        
    DB_QUEUE['players'][user_id]['bank_balance'] = new_bank
    return new_bank
...
async def background_cleanup_task():
    while True:
        ...
        try:
            ...
            # 6. Clear expired Earn Cooldowns
            expired_cooldowns = [k for k, v in EARN_COOLDOWNS.items() if now - v > 300]
            for k in expired_cooldowns:
                del EARN_COOLDOWNS[k]

            # 7. Clear expired Two-Step Earn Tasks
            expired_earn_tasks = [k for k, v in EARN_TASKS.items() if now - v.get('ts', now) > 3600]
            for k in expired_earn_tasks:
                del EARN_TASKS[k]

            # 7.5 Clear expired Pending Ad Claims (NEW)
            expired_claims = [k for k, v in PENDING_AD_CLAIMS.items() if now - v > 3600]
            for k in expired_claims:
                del PENDING_AD_CLAIMS[k]

            # 8. Clear expired Tower games (1 hour timeout)
            expired_towers = [k for k, v in ACTIVE_TOWER_GAMES.items() if now - v.get('ts', now) > 3600]
            for k in expired_towers:
                del ACTIVE_TOWER_GAMES[k]

            # 9. Clear stuck arenas safely
            expired_arenas = [k for k, v in ACTIVE_ARENAS.items() if now - v.get('ts', now) > 600]
            for k in expired_arenas:
                del ACTIVE_ARENAS[k]
...
def run_migrations():
    ...
    ADD COLUMN IF NOT EXISTS last_robbed_time TIMESTAMP WITH TIME ZONE,
    ...
    ADD COLUMN IF NOT EXISTS contest_score INTEGER DEFAULT 0,
    ...
    ADD COLUMN IF NOT EXISTS last_task_date TEXT,
    ...
    ADD COLUMN IF NOT EXISTS pub_balance NUMERIC DEFAULT 0,
    ...
    -- 2. NOW UPGRADE ECONOMY LIMITS TO INFINITE NUMERIC
...
def get_start_screen(user_id: int, section: str, is_private: bool, bot_username: str = None):
    ...
    if section == "main":
        ...
        text = (
            ...
            "<code>/arena [amt]</code> — The Underground Octagon (Group)\n" # 👈 ADD THIS LINE
            ...
            "<code>/tower [amt]</code> — The Syndicate Tower (Climb/Cashout)\n"
            "🏆 <b>SPORTS BETTING</b>\n"
...
def parse_bet(bet_str: str, balance: int) -> int:
    """Parses text like '1.5m', 'all', 'half' into exact integer values."""
    if not bet_str:
        raise ValueError("Empty bet")
        
    # Clean the input
    b = str(bet_str).lower().replace(",", "").strip()
...
async def cmd_rps(m: types.Message):
    ...
    args = m.text.split()
    if len(args) < 2: 
        return await m.answer(f"❌ <b>Usage:</b> <code>{args[0]} [amount]</code>", parse_mode="HTML")

    p_c = await get_player_fast(m.from_user) # Fetch the challenger
    try:
        amount = parse_bet(args[1], p_c['balance'])
        if amount < 100: return await m.answer("❌ Minimum bet is 100 Gold.")
    except ValueError:
        return await m.answer("❌ Invalid bet format. Use numbers, 'k', 'm', 'b', 'half', or 'all'.")

    if p_c['balance'] < amount:
        return await m.answer(f"❌ You don't have enough Gold! (Balance: {p_c['balance']:,})")
...
async def cmd_earn(m: types.Message):
    ...
    try:
        ...
        # 🌟 TWO-STEP UI: Store the Reward URL in RAM instead of showing it instantly
        EARN_TASKS[m.from_user.id] = {
            "reward_url": ad_data['reward_url'],
            "ts": now
        }

        ad_text = ad_data.get('text_html', 'Complete this sponsor task for gold!')
        kb = types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text=f"🚀 {ad_data.get('button_name', 'Go To Sponsor!')}", url=ad_data['click_url'])],
            [types.InlineKeyboardButton(text="✅ I Completed It", callback_data=f"earn_step2_{m.from_user.id}")]
        ])
        ...
        caption = (
            ...
            f"{ad_text}\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"<i>1️⃣ Click 'Go To Sponsor' and complete the task.</i>\n"
            f"<i>2️⃣ Return here and click 'I Completed It'.</i>\n"
...
@dp.callback_query(F.data.startswith("earn_step2_"))
async def handle_earn_step2(call: types.CallbackQuery):
    user_id = int(call.data.split("_")[2])
    
    if call.from_user.id != user_id:
        return await call.answer("❌ Not your task!", show_alert=True)
        
    task = EARN_TASKS.get(user_id)
    if not task:
        return await call.answer("❌ Task expired or already verified. Type /earn for a new one.", show_alert=True)
        
    # 🚨 SECURITY FIX: Enforce a minimum 12-second watch time
    time_passed = datetime.now().timestamp() - task['ts']
    if time_passed < 12:
        return await call.answer("❌ You didn't watch the ad! It takes at least 15 seconds. Keep watching!", show_alert=True)
        
    reward_url = task['reward_url']
    del EARN_TASKS[user_id] # Prevent double clicks
    
    # 🚨 SECURITY FIX: Issue the secure payload ticket
    PENDING_AD_CLAIMS[user_id] = datetime.now().timestamp()
    
    kb = types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(text="🎁 Claim Reward (Verification)", url=reward_url)]
    ])
    ...
    await call.message.answer(
        "✅ <b>𝗧𝗔𝗦𝗞 𝗠𝗔𝗥𝗞𝗘𝗗 𝗖𝗢𝗠𝗣𝗟𝗘𝗧𝗘𝗗</b>\n"
        "━━━━━━━━━━━━━━━━━━\n"
        "Final step! Click the button below to verify your completion with the Syndicate Ad Server.\n\n"
        "<i>(Note: This will briefly open your browser and instantly bounce you back to the vault with your payout!)</i>",
        reply_markup=kb,
        parse_mode="HTML"
    )
    await call.answer()
...
async def cmd_start(m: types.Message, command: CommandObject = None):
    ...
    if args:
        if args == "spin": return await cmd_spin(m)
            ...
            # 🚨 SECURITY FIX: Reject manual copy-paste exploits
            if m.from_user.id not in PENDING_AD_CLAIMS:
                return await m.answer("🛑 <b>SECURITY ALERT:</b>\nYou must generate and watch a valid contract via /earn first!", parse_mode="HTML")

            # Valid claim! Remove their ticket so they can't reuse it
            del PENDING_AD_CLAIMS[m.from_user.id]
...
async def cmd_deposit(m: types.Message):
    if m.from_user.id in active_games: return await m.answer("⏳ Please wait for your current action to finish...")
    
    args = m.text.split()
    if len(args) < 2: return await m.answer(f"❌ Usage: {args[0]} [amount]")
    
    active_games.add(m.from_user.id)
    try:
        p = await get_player_fast(m.from_user)
        try:
            amount = parse_bet(args[1], p.get('balance', 0))
            if amount <= 0: return await m.answer("❌ Amount must be greater than 0.")
        except ValueError:
            return await m.answer("❌ Invalid amount format. Use 'all', 'half', 'k', 'm', 'b'.")

        if p['balance'] < amount:
            return await m.answer("❌ You don't have that much Gold in your wallet!")

        await update_player_balance(m.from_user.id, -amount)
        await update_player_bank(m.from_user.id, amount)
        
        await m.answer(f"🏦 <b>DEPOSIT SUCCESSFUL</b>\nStashed <b>{amount:,} Gold</b> into your offshore bank.", parse_mode="HTML")
    finally:
        active_games.discard(m.from_user.id)
...
async def cmd_withdraw(m: types.Message):
    ...
    try:
        p = await get_player_fast(m.from_user)
        try:
            # Notice we pass bank_balance here so "all" pulls from the bank!
            amount = parse_bet(args[1], p.get('bank_balance', 0)) 
            if amount <= 0: return await m.answer("❌ Amount must be greater than 0.")
        except ValueError:
            return await m.answer("❌ Invalid amount format. Use 'all', 'half', 'k', 'm', 'b'.")

        if p.get('bank_balance', 0) < amount:
            return await m.answer("❌ You don't have that much Gold in your bank!")

        await update_player_bank(m.from_user.id, -amount)
        await update_player_balance(m.from_user.id, amount)
        
        await m.answer(f"🏦 <b>WITHDRAWAL SUCCESSFUL</b>\nPulled <b>{amount:,} Gold</b> out of your offshore bank.", parse_mode="HTML")
    finally:
        active_games.discard(m.from_user.id)
...
async def cmd_pay(m: types.Message):
    ...
    if target_user.id == m.from_user.id: return await m.answer("❌ You cannot send gold to yourself.")
    if target_user.is_bot: return await m.answer("❌ You cannot send gold to a bot.")
    ...
    try:
        ...
        try:
            amount = parse_bet(bet_str, sender['balance'])
            if amount < 100:
                return await m.answer("❌ The Syndicate doesn't process pocket change. Minimum transfer is <b>100 gold</b>.", parse_mode="HTML")
        except ValueError:
            return await m.answer("❌ Invalid amount format. Use numbers, 'k', 'm', 'b', 'half', or 'all'.")

        if sender['balance'] < amount:
            return await m.answer("❌ You do not have enough Gold for this transfer.")

        # Process Transfer
        await update_player_balance(m.from_user.id, -amount)
        await update_player_balance(target_user.id, amount)

        await m.answer(f"💸 <b>TRANSFER COMPLETE</b>\nSent <b>{amount:,} Gold</b> to {html.escape(target_user.first_name)}.", parse_mode="HTML")
    finally:
        active_games.discard(m.from_user.id)
...
async def cmd_coin(m: types.Message):
    ...
    try:
        p = await get_player_fast(m.from_user)
        try:
            amount = parse_bet(args[1], p['balance'])
...
async def cmd_crates(m: types.Message):
    ...
    # Figure out which argument is the crates and which is the bet string
    if args[1].isdigit() and 3 <= int(args[1]) <= 9:
        c_count, bet_str = int(args[1]), args[2]
    elif args[2].isdigit() and 3 <= int(args[2]) <= 9:
        c_count, bet_str = int(args[2]), args[1]
    else: 
        return await m.answer("❌ You must spawn between <b>3 and 9 crates</b>.", parse_mode="HTML")
    ...
    try:
        ...
        try:
            amount = parse_bet(bet_str, p['balance'])
            if amount <= 0: return await m.answer("❌ Invalid bet amount.")
        except ValueError:
            return await m.answer("❌ Invalid bet format. Use numbers, 'k', 'm', 'b', 'half', or 'all'.")
...
async def cmd_mines(m: types.Message):
    ...
    args = m.text.split()
    if len(args) < 3 or not args[2].isdigit(): return await m.answer("❌ Usage: /mines [amount] [mines (1-24)]")
...
async def cmd_aeroplane(m: types.Message):
    ...
    if p['balance'] < amount:
        return await m.answer(f"❌ You don't have enough Gold! (Balance: {p['balance']:,})")
...
async def cmd_tower(m: types.Message, command: CommandObject = None):
    if m.from_user.id in ACTIVE_TOWER_GAMES:
        return await m.answer("🛑 You are already in the Tower! Finish your current climb or cash out.")

    if not command.args:
        return await m.answer("❌ <b>Format:</b> <code>/tower [amount]</code>\n<i>Example: /tower 5k, /tower half, /tower all</i>", parse_mode="HTML")

    p = await get_player_fast(m.from_user)
    try:
        bet_amount = parse_bet(command.args, p['balance'])
        if bet_amount <= 0: return await m.answer("❌ Invalid bet amount.")
    except ValueError:
        return await m.answer("❌ Invalid bet format. Use numbers, 'k', 'm', 'b', 'half', or 'all'.")
    ...
    # Deduct initial bet
    await update_player_balance(m.from_user.id, -bet_amount)

    # Initialize Game
    ACTIVE_TOWER_GAMES[m.from_user.id] = {
        "bet": bet_amount,
        "floor": 1,
        "ts": datetime.now().timestamp()
    }

    kb = generate_tower_keyboard(m.from_user.id, 1)
    await m.answer(
        f"🏢 <b>THE SYNDICATE TOWER</b> 🏢\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"💰 <b>Initial Bet:</b> {bet_amount:,} Gold\n"
        f"📈 <b>Current Multiplier:</b> {TOWER_MULTIPLIERS[1]}x\n"
        f"⚠️ <b>Risk:</b> 1 Bomb hidden among 3 Vaults.\n\n"
        f"<b>FLOOR 1:</b> Pick a vault to open!",
        reply_markup=kb,
        parse_mode="HTML"
    )

@dp.callback_query(F.data.startswith("tower_door_"))
async def handle_tower_door(call: types.CallbackQuery):
    user_id = int(call.data.split("_")[2])
    if call.from_user.id != user_id:
        return await call.answer("❌ This is not your tower climb!", show_alert=True)

    game = ACTIVE_TOWER_GAMES.get(user_id)
    if not game:
        return await call.answer("❌ Game expired or already finished.", show_alert=True)

    bet = game['bet']
    current_floor = game['floor']

    # 🎲 THE 50/50 ROLL (2 Safe / 2 Police)
    if random.random() < 0.50:
        # 🚨 BUSTED!
        del ACTIVE_TOWER_GAMES[user_id]
        
        # We strip the buttons so they can't click them anymore
        ...
        return await call.message.answer(
            f"🚨 <b>SWAT TEAM!</b>\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"You kicked down the wrong door on Floor {current_floor + 1}.\n"
            f"You were arrested and lost <b>{bet:,} Gold</b>.",
            parse_mode="HTML"
        )
    ...
    current_multiplier = TOWER_MULTIPLIERS[new_floor]
    current_winnings = int(bet * current_multiplier)

    # 🏆 JACKPOT CHECK (Floor 6 Cleared)
    if new_floor == 6:
        del ACTIVE_TOWER_GAMES[user_id]
        await update_player_balance(user_id, current_winnings)
        ...
        # --- ADD THE AUDIT LOG HERE ---
        safe_name = html.escape(call.from_user.first_name)
        asyncio.create_task(send_log(f"🏢 <b>TOWER JACKPOT</b>\n<a href='tg://user?id={user_id}'>{safe_name}</a> cleared Floor 6 and won <b>{current_winnings:,} Gold</b> ({current_multiplier}x)!"))

        return await call.message.answer(
            f"🚁 <b>PENTHOUSE REACHED!</b>\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"You broke through to the roof and escaped on the Syndicate chopper!\n\n"
            f"💰 <b>Jackpot Won:</b> {current_winnings:,} Gold ({current_multiplier}x)",
            parse_mode="HTML"
        )

    # 🧗 CLIMBING (Generate next floor)
    kb = types.InlineKeyboardMarkup(inline_keyboard=[
        [
            types.InlineKeyboardButton(text="🚪 1", callback_data=f"tower_door_{user_id}"),
            types.InlineKeyboardButton(text="🚪 2", callback_data=f"tower_door_{user_id}"),
            types.InlineKeyboardButton(text="🚪 3", callback_data=f"tower_door_{user_id}"),
            types.InlineKeyboardButton(text="🚪 4", callback_data=f"tower_door_{user_id}")
        ],
        [
            types.InlineKeyboardButton(text=f"💰 Cash Out ({current_winnings:,})", callback_data=f"tower_cashout_{user_id}")
        ]
    ])
    ...
    await call.message.edit_text(
        f"✅ <b>CLEAR!</b>\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"💰 <b>Current Value:</b> {current_winnings:,} Gold ({current_multiplier}x)\n"
        f"📍 <b>Floor:</b> {new_floor + 1}/6\n\n"
        f"<i>Do you want to risk it all to push higher, or cash out now?</i>",
        reply_markup=kb,
        parse_mode="HTML"
    )

@dp.callback_query(F.data.startswith("tower_cashout_"))
async def handle_tower_cashout(call: types.CallbackQuery):
    user_id = int(call.data.split("_")[2])
    if call.from_user.id != user_id:
        return await call.answer("❌ Not your game!", show_alert=True)
    ...
    game = ACTIVE_TOWER_GAMES.get(user_id)
    if not game:
        return await call.answer("❌ Game expired.", show_alert=True)

    winnings = int(game['bet'] * TOWER_MULTIPLIERS[game['floor']])
    del ACTIVE_TOWER_GAMES[user_id]
    ...
    # --- ADD THE AUDIT LOG HERE ---
    safe_name = html.escape(call.from_user.first_name)
    asyncio.create_task(send_log(f"🏢 <b>TOWER CASHOUT</b>\n<a href='tg://user?id={user_id}'>{safe_name}</a> bailed out on Floor {game['floor']} with <b>{winnings:,} Gold</b>!"))

    await call.message.answer(
        f"💼 <b>SECURED THE BAG</b>\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"You bailed out early and escaped the Tower safely.\n"
        f"💰 <b>Profit Added:</b> {winnings:,} Gold",
        parse_mode="HTML"
    )

@dp.message(Command("arena"))
async def cmd_arena(m: types.Message, command: CommandObject = None):
    if m.chat.id in ACTIVE_ARENAS:
        return await m.answer("🛑 The Octagon is already occupied! Wait for the current fight to finish.")

    if not command.args:
        return await m.answer("❌ <b>Format:</b> <code>/arena [amount]</code>\n<i>Examples: /arena 50k, /arena 1.5m, /arena all</i>", parse_mode="HTML")

    # 1. Fetch player early so the Parser knows their vault balance
    p = await get_player_fast(m.from_user)

    # 2. Run the text through the Universal Parser
    try:
        bet_amount = parse_bet(command.args, p['balance'])
        if bet_amount < 100: 
            return await m.answer("❌ Minimum bet is 100 Gold.")
        if bet_amount > p['balance']:
            return await m.answer(f"❌ You don't have enough Gold! (Balance: {p['balance']:,})")
    except ValueError:
        return await m.answer("❌ Invalid bet amount. Use numbers, 'k', 'm', 'b', or 'all'.")

    red_name = f"The {random.choice(ARENA_ADJECTIVES)} {random.choice(ARENA_NOUNS)}"
    blue_name = f"The {random.choice(ARENA_ADJECTIVES)} {random.choice(ARENA_NOUNS)}"

    # Initialize Pari-Mutuel Pool (Empty at the start)
    ACTIVE_ARENAS[m.chat.id] = {
        "status": "booking",
        "bet_amount": bet_amount, # Stores the required buy-in for this room
        "red_name": red_name,
        "blue_name": blue_name,
        "red_bets": {},
        "blue_bets": {},
    ...
    kb = types.InlineKeyboardMarkup(inline_keyboard=[
        [
            types.InlineKeyboardButton(text="🔴 Bet RED", callback_data=f"arena_bet_red"),
            types.InlineKeyboardButton(text="🔵 Bet BLUE", callback_data=f"arena_bet_blue")
        ]
    ])

    sent_msg = await m.answer(
        f"🩸 <b>THE UNDERGROUND OCTAGON</b> 🩸\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"The cage doors are open! Place your bets.\n"
        f"⏳ <i>Match starts in 30 seconds...</i>\n\n"
        f"🔴 <b>RED:</b> {red_name}\n"
        f"🔵 <b>BLUE:</b> {blue_name}\n\n"
        f"🎟️ <b>Bet Size:</b> {bet_amount:,} Gold\n"
        f"💰 <b>Total Pool:</b> 0 Gold\n"
        f"<i>(Note: Odds shift dynamically as players bet!)</i>",
        reply_markup=kb,
        parse_mode="HTML"
    )

    # Start the fight engine in the background
    asyncio.create_task(run_arena_fight(m.chat.id, sent_msg))

@dp.callback_query(F.data.startswith("arena_bet_"))
async def handle_arena_bet(call: types.CallbackQuery):
    chat_id = call.message.chat.id
    arena = ACTIVE_ARENAS.get(chat_id)

    if not arena or arena['status'] != "booking":
        return await call.answer("❌ Bets are closed or fight ended!", show_alert=True)

    user_id = call.from_user.id
    if user_id in arena['red_bets'] or user_id in arena['blue_bets']:
        return await call.answer("❌ You already placed a bet on this fight!", show_alert=True)

    # Pull the exact bet size defined when the room was created
    bet_amount = arena['bet_amount']

    p = await get_player_fast(call.from_user)
    if p['balance'] < bet_amount:
        return await call.answer(f"❌ You need {bet_amount:,} Gold to join the pool!", show_alert=True)

    await update_player_balance(user_id, -bet_amount)

    side = call.data.split("_")[2] # "red" or "blue"
    arena[f"{side}_bets"][user_id] = {"bet": bet_amount, "name": call.from_user.first_name}

    total_pool = sum(b['bet'] for b in arena['red_bets'].values()) + sum(b['bet'] for b in arena['blue_bets'].values())
    
    await call.answer(f"✅ Bet placed on {side.upper()}!", show_alert=False)
    
    # Update the message so everyone sees the pool grow
    try:
        kb = call.message.reply_markup
        await call.message.edit_text(
            f"🩸 <b>THE UNDERGROUND OCTAGON</b> 🩸\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"🔴 <b>RED:</b> {arena['red_name']} ({len(arena['red_bets'])} bets)\n"
            f"🔵 <b>BLUE:</b> {arena['blue_name']} ({len(arena['blue_bets'])} bets)\n\n"
            f"🎟️ <b>Bet Size:</b> {bet_amount:,} Gold\n"
            f"💰 <b>Total Pool:</b> {total_pool:,} Gold\n"
            f"🔥 <i><a href='tg://user?id={user_id}'>{html.escape(call.from_user.first_name)}</a> just bet on {side.upper()}!</i>",
            reply_markup=kb,
            parse_mode="HTML"
        )
    except: pass

async def run_arena_fight(chat_id: int, msg: types.Message):
    await asyncio.sleep(30) # 30 Second Betting Window
    ...
    # Lock the cage (remove buttons)
    total_red = sum(b['bet'] for b in arena['red_bets'].values())
    total_blue = sum(b['bet'] for b in arena['blue_bets'].values())
    total_pool = total_red + total_blue
    
    # --- ADD THIS CHECK ---
    if total_pool == 0:
        await msg.edit_text("🤡 <b>FIGHT CANCELLED</b>\nNobody placed any bets. The crowd went home.", parse_mode="HTML")
        del ACTIVE_ARENAS[chat_id]
        return
    # ----------------------

    net_pool = int(total_pool * 0.95) # The House takes 5%
    
    await msg.edit_text(
        f"🛑 <b>BETS LOCKED. THE CAGE IS CLOSED.</b>\n"
        f"💰 <b>Final Pool:</b> {total_pool:,} Gold\n"
        f"<i>(The Syndicate takes its 5% cut)</i>\n\n"
        f"Let's bleed.",
        parse_mode="HTML"
    )
    await asyncio.sleep(3)
    ...
    # --- 3 ROUNDS ---
    for round_num in range(1, 4):
        red_text, red_pts = get_mma_move(arena['red_name'])
        blue_text, blue_pts = get_mma_move(arena['blue_name'])
        ...
        await msg.edit_text(
            ...
            f"🔴 {red_text}\n"
            f"🔵 {blue_text}\n\n"
            f"📊 <b>SCORE:</b> 🔴 {red_score} - {blue_score} 🔵",
            parse_mode="HTML"
        )
        await asyncio.sleep(3)
    ...
    if red_score == blue_score:
        await msg.edit_text(
            f"🚨 <b>SCORE TIED! SUDDEN DEATH!</b> 🚨\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"Next fighter to draw blood takes the pot!",
            parse_mode="HTML"
        )
        await asyncio.sleep(3)
        
        # Keep flipping a coin until someone wins
        while red_score == blue_score:
            if random.random() < 0.5:
                ot_text, ot_pts = get_mma_move(arena['red_name'])
                red_score += ot_pts
                winner_color, winner_text = "RED", f"🔴 {ot_text}"
            else:
                ot_text, ot_pts = get_mma_move(arena['blue_name'])
                blue_score += ot_pts
                winner_color, winner_text = "BLUE", f"🔵 {ot_text}"
                
        await msg.edit_text(f"☠️ <b>OVERTIME:</b>\n{winner_text}", parse_mode="HTML")
        await asyncio.sleep(2)

    # --- PAYOUT PHASE ---
    red_won = red_score > blue_score
    winner_name = arena['red_name'] if red_won else arena['blue_name']
    winner_icon = "🔴" if red_won else "🔵"
    winning_bets = arena['red_bets'] if red_won else arena['blue_bets']
    losing_bets = arena['blue_bets'] if red_won else arena['red_bets']

    final_text = (
        f"🏆 <b>THE FIGHT IS OVER!</b>\n"
        f"━━━━━━━━━━━━━━━━━━\n"
        f"{winner_icon} <b>{winner_name} wins!</b> ({red_score} - {blue_score})\n\n"
    )

    # Calculate Pari-Mutuel Payouts
    if not winning_bets:
        final_text += "🤡 <b>Nobody bet on the winner!</b> The Syndicate sweeps the entire pot."
        # 🚨 LOGGING: The House sweeps the pot
        asyncio.create_task(send_log(f"🩸 <b>ARENA SWEEP</b>\nNobody bet on {winner_name}. The House swept the <b>{net_pool:,} Gold</b> pot!"))
        
    elif not losing_bets:
        # Everyone bet on the same person. Refund them so the House doesn't steal 5% for no reason.
        final_text += "<i>Everyone bet on the winner. Bets refunded.</i>\n"
        for uid, data in winning_bets.items():
            await update_player_balance(uid, data['bet'])
            
    else:
        final_text += "💸 <b>PAYOUTS:</b>\n"
        winning_pool_size = sum(b['bet'] for b in winning_bets.values())
        
        for uid, data in winning_bets.items():
            # Percentage of the winning pool they own * Net Total Pool
            player_share = data['bet'] / winning_pool_size 
            payout = int(player_share * net_pool)
            
            await update_player_balance(uid, payout)
            final_text += f"• {html.escape(data['name'])} won <b>{payout:,}</b> Gold!\n"
            
        # 🚨 LOGGING: Massive player payouts
        asyncio.create_task(send_log(f"🩸 <b>ARENA PAYOUT</b>\n{winner_name} won the fight! <b>{net_pool:,} Gold</b> distributed among {len(winning_bets)} winners."))
...
async def main():
    ...
    commands = [
        ...
        types.BotCommand(command="arena", description="🥊 The Underground Octagon (Group)"),
        ...
        types.BotCommand(command="tower", description="🏢 The Syndicate Tower (50/50)"), # 👈 ADD THIS LINE
