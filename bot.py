...
async def cmd_broadcast(m: types.Message):
    ...
    for target_id in all_targets:
        try:
            ...
            # 👇 ADD THIS LINE to safely space out the broadcast!
            await asyncio.sleep(0.05) 
...
async def handle_pk_start(call: types.CallbackQuery):
    ...
    # THE FIX: ADD THIS 3 SECOND DELAY!
    await asyncio.sleep(3.0) 
...
async def pk_afk_timer(host_id: int, expected_ts: float, player_id: int):
    ...
    if game and game.get('status') == 'playing' and game.get('expected_ts') == expected_ts:
        ...
        # THE FIX: INCREASED FROM 2.5 TO 3.5 SECONDS
        await asyncio.sleep(3.5) 
        asyncio.create_task(run_poker_turn(host_id))
...
async def handle_pk_action(call: types.CallbackQuery):
    ...
    try:
        ...
        # THE FIX: INCREASED FROM 2.0 TO 3.5 SECONDS
        await asyncio.sleep(3.5) 
        asyncio.create_task(run_poker_turn(host_id))
...
async def resolve_heist_stage(host_id: int, expected_ts: float):
    ...
    for pid in game['active']:
        ...
        if choice == "run":
            ...
            game['dead_drop'] += int(game['bet'] * 0.20) # Cops seize 80% of the money
            round_logs.append(f"💀 <b>@{pname}</b> froze in fear and was arrested! <i>(Cops seized 80% of their stash)</i>")
            ...
            if random.random() < stage_data['cops']:
                game['busted'].append(pid)
                game['dead_drop'] += int(game['bet'] * 0.20) # Cops seize 80% of the money
                round_logs.append(f"🚨 <b>@{pname}</b> pushed too deep and got BUSTED! <i>(Cops seized 80% of their stash)</i>")
            else:
...
async def finish_heist(host_id: int):
    ...
    # If anyone survived Stage 6, they are automatically runners at 25x
    for pid in game['active']:
        game['runners'][pid] = {"name": game['names'][pid], "mult": 25.0} # Fixed to 25.0
...
async def cmd_tower(m: types.Message, command: CommandObject = None):
    ...
    kb = types.InlineKeyboardMarkup(inline_keyboard=[
        [
            types.InlineKeyboardButton(text="🚪 1", callback_data=f"tower_door_{m.from_user.id}"),
            types.InlineKeyboardButton(text="🚪 2", callback_data=f"tower_door_{m.from_user.id}"),
            types.InlineKeyboardButton(text="🚪 3", callback_data=f"tower_door_{m.from_user.id}"),
            types.InlineKeyboardButton(text="🚪 4", callback_data=f"tower_door_{m.from_user.id}")
        ]
    ])
    ...
    await m.answer(
        ...
        f"━━━━━━━━━━━━━━━━━━\n"
        f"💰 <b>Bet:</b> {bet_amount:,} Gold\n"
        f"📍 <b>Current Floor:</b> Lobby (1/6)\n\n"
        f"<i>Two doors are safe. Two are rigged by the Police.\n"
        f"Pick a door to climb to the next floor.</i>",
        reply_markup=kb,
        parse_mode="HTML"
    )
...
async def cmd_arena(m: types.Message, command: CommandObject = None):
    ...
    if not command.args:
        return await m.answer("❌ <b>Format:</b> <code>/arena [bet amount]</code>\n<i>Example: /arena 5000</i>", parse_mode="HTML")

    try:
        bet_amount = int(command.args)
        if bet_amount < 100: return await m.answer("❌ Minimum bet is 100 Gold.")
    except:
        return await m.answer("❌ Invalid bet amount.")
