...
async def cmd_start(m: types.Message, command: CommandObject = None):
    ...
    if args:
        if args == "spin": return await cmd_spin(m)
            ...
            # Daily Cap of 10 ads to protect economy
            count = p.get('tasks_completed', 0) if p.get('last_task_date') == today_str else 0
            if count >= 10:
                return await m.answer("🛑 <b>Limit Reached!</b>\nYou've finished your 10 daily contracts. Come back tomorrow!")
            ...
            return await m.answer(
                f"✅ <b>𝗖𝗢𝗡𝗧𝗥𝗔𝗖𝗧 𝗣𝗔𝗜𝗗</b>\n"
                f"Sponsor view confirmed.\n"
                f"💰 <b>{reward:,} Gold</b> added to your vault!\n"
                f"<i>({count + 1}/10 today)</i>",
                parse_mode="HTML"
            )
