...
def generate_ad_link(user_id: int, action: str):
    ...
    AD_TOKENS[ticket] = {"user_id": user_id, "action": action}
...
async def background_cleanup_task():
    while True:
        ...
        try:
            ...
            for d in [mine_games, card_games, dice_games, coin_games, rps_games, crate_games, challenge_games]:
...
async def handle_brawl_move(call: types.CallbackQuery):
    ...
    if game['c_idx'] >= 3 or game['d_idx'] >= 3:
        ...
        if winner:
            pot = int(game['bet'] * 2 * 0.95) # 5% House Tax
            p_win = await get_player_fast(types.User(id=winner, is_bot=False, first_name=""))
            await update_player_balance(winner, pot, {"wins": p_win.get('wins', 0) + 1})
...
async def cmd_start(m: types.Message, command: CommandObject = None):
    ...
    # --- 🚨 AD TICKET INTERCEPTOR & REWARD PAYOUT 🚨 ---
    if args and args in AD_TOKENS:
        session = AD_TOKENS.pop(args) # Destroys ticket instantly so it can't be reused!
        
        if session['user_id'] != m.from_user.id:
            return await m.answer("❌ Invalid or expired ad session.")
        ...
        if action == "spin":
            ...
            return await m.answer("✅ <b>500 Coins Collected!</b>\nWant to turn it into <b>1,000</b>? Watch one more ad!", reply_markup=double_kb, parse_mode="HTML")
            
        elif action == "daily_double":
            await update_player_balance(m.from_user.id, 500)
            return await m.answer("🔥 <b>DOUBLE CLAIMED!</b>\nYou got an extra 500 gold!", parse_mode="HTML")
            ...
            if count < 5:
                ...
                return await m.answer("🏦 Your daily bank tiers are exhausted!")
...
async def cmd_daily(m: types.Message):
    ...
    try:
        ...
        if m.chat.type == "private":
            kb = types.InlineKeyboardMarkup(inline_keyboard=[[
                 types.InlineKeyboardButton(text="📺 Claim 500 (Watch Ad)", web_app=types.WebAppInfo(url=generate_ad_link(m.from_user.id, "daily")))
            ]])
        else:
            bot_user = await bot.get_me()
            kb = types.InlineKeyboardMarkup(inline_keyboard=[[
...
async def cmd_bank(m: types.Message):
    ...
    try:
        ...
        if m.chat.type == "private":
            kb = types.InlineKeyboardMarkup(inline_keyboard=[[
                types.InlineKeyboardButton(text=f"📺 Tier {count+1}/5 ({reward} coins)", web_app=types.WebAppInfo(url=generate_ad_link(m.from_user.id, "bank")))
            ]])
        else:
            bot_user = await bot.get_me()
            kb = types.InlineKeyboardMarkup(inline_keyboard=[[
...
async def pvp_afk_timer(owner_id: int):
    """Prevents players from holding the pot hostage if they close the app."""
    await asyncio.sleep(60)
    game = challenge_games.get(owner_id)
    
    # If the game is still stuck waiting for a choice
    if game and game.get('status') == 'playing' and not game.get('choice_made'):
...
async def route_pvp_game(msg: types.Message, game: dict):
    ...
    if bot_msg:
        ...
        asyncio.create_task(pvp_afk_timer(game['challenger']))
