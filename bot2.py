...
# --- ADSGRAM CREDENTIALS ---
ADSGRAM_BLOCK_ID = "23671"
ADSGRAM_TOKEN = "b1231df256914e13936b24ad03576743"
...
async def cmd_earn(m: types.Message):
    ...
    try:
        ...
        # Ensure Block ID is numeric only
        ...
        # 10s timeout prevents the "timed out" error from crashing the command
        ...
        caption = (
            ...
            f"{ad_text}\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"<i>Complete the task, then click claim to receive 25,000 Gold.</i>"
        )
        ...
        await m.answer("❌ Connection to Ad-Server timed out. Try again in a moment.")
    except Exception as e:
...
async def cmd_start(m: types.Message, command: CommandObject = None):
    ...
    if args:
        if args == "spin": return await cmd_spin(m)
            ...
            # Use tasks_completed if you added it to DB, otherwise use a local count
            count = p.get('tasks_completed', 0) if p.get('last_task_date') == today_str else 0
            if count >= 10:
                return await m.answer("🛑 <b>Daily Limit Reached</b>\nYou've finished your 10 daily contracts.")

            reward = 25000
            # Note: Ensure 'tasks_completed' and 'last_task_date' exist in your Supabase schema!
